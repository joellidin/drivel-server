...
gcloud run deploy ${CLOUD_RUN_SERVICE} --image ${GCR_IMAGE_NAME} --region ${GCR_REGION} --allow-unauthenticated --update-secrets=${SECRETS_FOLDER}/api-key/${GCP_SECRET_NAME_OPENAI_KEY}=projects/${GCP_PROJECT_NUMBER}/secrets/${GCP_SECRET_NAME_OPENAI_KEY}:${SECRET_VERSION},${SECRETS_FOLDER}/org-id/${GCP_SECRET_NAME_OPENAI_ORGANIZATION_ID}=projects/${GCP_PROJECT_NUMBER}/secrets/${GCP_SECRET_NAME_OPENAI_ORGANIZATION_ID}:${SECRET_VERSION}
